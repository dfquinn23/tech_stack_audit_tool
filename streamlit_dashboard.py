...
def new_audit_page():
    ...
    if st.checkbox("Or enter tools manually"):
        with st.expander("Manual Tool Entry"):
            ...
            with col3:
                users = st.text_input(
                    "Used By", placeholder="Portfolio Management")
            with col4:
    ...
    if st.button("🚀 Start Complete Audit", type="primary", use_container_width=True):
        ...
        # Run the audit
        run_audit(client_name, client_domain, uploaded_file,
                  enable_discovery, auto_advance)


def run_audit(client_name, client_domain, uploaded_file, enable_discovery, auto_advance):
    """Run the complete audit process - FIXED VERSION"""
    ...
    try:
        # Handle file upload properly - FIXED LOGIC
        if uploaded_file is not None:
            ...
            try:
                ...
                csv_path = f"data/temp_{timestamp}.csv"
                
                # Ensure data directory exists
                Path("data").mkdir(exist_ok=True)
                
                # Save the DataFrame (not the uploaded file buffer)
                df.to_csv(csv_path, index=False)
                
                st.success(f"✅ File processed successfully! ({len(df)} tools loaded)")
                st.write("**Tools to be processed:**")
                st.write(df[['Tool Name', 'Category']].head(10))
                if len(df) > 10:
                    st.write(f"... and {len(df) - 10} more tools")
            ...
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            csv_path = f"data/manual_{timestamp}.csv"
            df = pd.DataFrame(st.session_state.manual_tools)
            df.to_csv(csv_path, index=False)
            
            st.success(f"✅ Manual tools processed! ({len(df)} tools loaded)")
        
        else:
            # Use default file
            csv_path = "data/tech_stack_list.csv"
            st.info("ℹ️ Using default tool inventory")
        ...
        try:
            ...
            if success:
                ...
                status_text.error(
                    "❌ Audit failed - check the logs for details")

        finally:
            loop.close()

        # Cleanup temp files
        if csv_path and "temp_" in csv_path and Path(csv_path).exists():
...
def display_audit_results(pipeline):
    ...
    with col4:
        annual_value = gap_analysis.get('analysis_summary', {}).get(
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["🔍 Discovery", "🔗 Integrations", "🤖 Opportunities", "📋 Report"])
                ...
                with st.expander(f"{i}. {opp.get('name', 'Unknown Opportunity')}"):
                    ...
                    with col1:
                        st.metric("Priority Score",
                                  f"{opp.get('priority_score', 0)}/25")
                    with col2:
                        st.metric("Annual Savings",
                                  f"${opp.get('roi_estimate', 0):,}")
                    with col3:
                        st.metric("Implementation", opp.get(
                            'implementation_effort', 'Unknown'))

                    if 'n8n_workflow' in opp:
                        st.write(
                            "**n8n Workflow:**", opp['n8n_workflow'].get('description', 'No description'))
        else:
    ...
    with tab4:
        ...
        if output_dir.exists():
            report_files = list(output_dir.glob(
                f"*{pipeline.stage_manager.audit_id}*"))
...
OPENAI_MODEL=gpt-4
    ...
    with st.expander("Default Audit Settings"):
        default_auto_discovery = st.checkbox(
            "Enable auto-discovery by default", value=True)
        default_auto_advance = st.checkbox(
            "Auto-advance stages by default", value=True)
