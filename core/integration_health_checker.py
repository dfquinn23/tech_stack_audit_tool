...
class IntegrationHealthChecker:
    ...
    def __init__(self):
        self.cache_duration = timedelta(hours=6)  # Cache health checks for 6 hours
    ...
    async def assess_integration_health(self, source_tool: str, target_tool: str, 
        ...
        if current_integration_data:
            ...
            if expected_pattern:
                ...
                assessment.issues_found.append("Expected integration not found")
                assessment.recommendations.append(f"Implement {expected_pattern['expected_type'].value} integration")
            else:
                assessment.status = IntegrationStatus.UNKNOWN
                assessment.issues_found.append("No integration pattern defined")
        ...
        assessment.recommendations.extend(self._generate_recommendations(assessment, expected_pattern))
        
        # Cache the result
        self._save_cache(cache_key, assessment.__dict__)
        
        print(f"   Status: {assessment.status.value}, Health Score: {assessment.health_score}/100")
    ...
    async def _analyze_existing_integration(self, assessment: IntegrationAssessment, integration_data: Dict):
        ...
        if "status" in integration_data:
            try:
                assessment.status = IntegrationStatus(integration_data["status"].lower())
            except ValueError:
                assessment.status = IntegrationStatus.UNKNOWN
        
        if "integration_type" in integration_data:
            try:
                assessment.integration_type = IntegrationType(integration_data["integration_type"].lower())
            except ValueError:
                assessment.integration_type = IntegrationType.UNKNOWN
        
        if "health_score" in integration_data:
            assessment.health_score = max(0, min(100, int(integration_data.get("health_score", 0))))
        
        if "error_rate" in integration_data:
            assessment.error_rate = max(0.0, min(1.0, float(integration_data.get("error_rate", 0.0))))
        
        if "last_sync" in integration_data:
            try:
                assessment.last_sync = datetime.fromisoformat(integration_data["last_sync"])
        ...
        if assessment.last_sync:
            ...
            if days_since_sync > 7:
                assessment.issues_found.append(f"Last sync was {days_since_sync} days ago")
            elif days_since_sync > 1:
                assessment.issues_found.append(f"Sync may be delayed ({days_since_sync} days)")
        
        # Analyze error rates
        if assessment.error_rate > 0.1:  # 10%+ error rate
            assessment.issues_found.append(f"High error rate: {assessment.error_rate:.1%}")
        elif assessment.error_rate > 0.05:  # 5%+ error rate
            assessment.issues_found.append(f"Elevated error rate: {assessment.error_rate:.1%}")
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
                    ...
                    if source_endpoint:
                        ...
                        if not source_healthy:
                            assessment.issues_found.append(f"Source API ({assessment.source_tool}) unreachable")
                    
                    # Test target API  
                    if target_endpoint:
                        target_healthy = await self._test_api_endpoint(session, target_endpoint)
                        if not target_healthy:
                            assessment.issues_found.append(f"Target API ({assessment.target_tool}) unreachable")
            
            except Exception as e:
                assessment.issues_found.append(f"API health check failed: {str(e)}")
    ...
    def _calculate_health_score(self, assessment: IntegrationAssessment) -> int:
        ...
        if assessment.error_rate > 0:
            base_score -= (assessment.error_rate * 50)  # High error rate reduces score significantly
    ...
    def _generate_recommendations(self, assessment: IntegrationAssessment, expected_pattern: Optional[Dict]) -> List[str]:
        ...
        if assessment.status == IntegrationStatus.MISSING and expected_pattern:
            recommendations.append(f"Implement {expected_pattern['expected_type'].value} integration between {assessment.source_tool} and {assessment.target_tool}")
            recommendations.append(f"Expected business value: Improved {expected_pattern['data_flow']} data flow")
            
            # Add common issue prevention
            common_issues = expected_pattern.get("common_issues", [])
            if common_issues:
                recommendations.append(f"Prevent common issues: {', '.join(common_issues)}")
        
        elif assessment.status == IntegrationStatus.BROKEN:
            recommendations.append(f"Investigate and repair broken integration")
            recommendations.append(f"Review error logs and authentication status")
            if assessment.error_rate > 0.2:
                recommendations.append(f"Address high error rate ({assessment.error_rate:.1%})")
            ...
            if assessment.error_rate > 0.05:
                recommendations.append(f"Reduce error rate from {assessment.error_rate:.1%}")
            if assessment.last_sync and (datetime.now() - assessment.last_sync).days > 3:
                recommendations.append(f"Improve sync frequency (last sync: {assessment.last_sync.strftime('%Y-%m-%d')})")
            ...
            if assessment.health_score < 80:
                recommendations.append(f"Consider optimization opportunities to improve health score")
        
        # Add business impact recommendations
        if assessment.business_criticality == "high" and assessment.health_score < 70:
            recommendations.append(f"HIGH PRIORITY: Critical business integration needs immediate attention")
    ...
    async def assess_complete_integration_matrix(self, tool_inventory: Dict[str, dict]) -> Dict[str, IntegrationAssessment]:
        ...
        for i, source_tool in enumerate(tools):
            for j, target_tool in enumerate(tools):
                if i < j:  # Avoid duplicate pairs and self-integration
                    tasks.append(self.assess_integration_health(source_tool, target_tool))
        ...
        for (source_tool, target_tool), result in zip(tool_pairs, results):
            if isinstance(result, IntegrationAssessment):
                ...
                print(f"⚠️ Assessment failed for {source_tool}→{target_tool}: {result}")
        
        print(f"✅ Integration matrix completed: {len(assessments)} assessments")
    ...
    def generate_integration_summary(self, assessments: Dict[str, IntegrationAssessment]) -> Dict[str, Any]:
        ...
        critical_issues = [
            {"integration": key, "health_score": assessment.health_score, "status": assessment.status.value}
